/*
  ...
  ## 2. New Tables
    - `documents`
      ...
      - `embedding` (vector(768)): 768-dimensional vector embedding using
        Google's `gemini-embedding-001` model with `outputDimensionality=768`
        (Matryoshka representation learning, so the 768-d slice is a valid
        truncated form of the model's full 3072-d output).
  ...
  ## NOTE on dimensionality
    Earlier revisions of this migration used `vector(384)` to match the
    HuggingFace `sentence-transformers/all-MiniLM-L6-v2` model. The runtime
    has since moved to Google's `gemini-embedding-001` at 768 dims. If you
    are migrating an existing 384-d database, run:
    ...
    and then re-ingest with `pnpm tsx scripts/add-books.ts`.
*/
